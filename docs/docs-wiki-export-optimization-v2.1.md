...
#### Phase 3-A: 正确性保障（P0 - 必做）

| 任务 ID | 任务描述 | 实现要点 | 预计工作量 |
|---------|----------|----------|------------|
| **T3.A1** | 锚点冲突检测 | 构建全局 anchor map，统计重复锚点数量 | 小 |
| **T3.A2** | 锚点自动去重 | 遇到重复时自动加后缀（`#install` → `#install-2`） | 中 |
| **T3.A3** | 全局链接修复 | 同步修复 TOC 和页内链接指向新锚点 | 中 |
| **T3.A4** | 冲突可观测性 | 输出冲突统计；新增 `--warn-anchor-collisions` | 小 |

**价值**：不修复则"目录存在但跳不到正文"，属于功能性缺陷。

#### Phase 3-B: 体验提升（P1 - 高价值）

| 任务 ID | 任务描述 | 实现要点 | 预计工作量 |
|---------|----------|----------|------------|
| **T3.B1** | 双版本输出 | `--split-output DIR` 同时输出 merged + split 版本 | 中 |
| **T3.B2** | 分文件索引 | 生成 `INDEX.md`（结构索引）+ 各页独立文件 | 小 |
| **T3.B3** | 统一 assets | 合并/分文件共用同一 assets 目录 | 小 |
| **T3.B4** | 合并格式优化 | 结构统一：全局目录 → 分隔线 → 章节 | 小 |
...
# Phase 3-A（计划中）
--warn-anchor-collisions    # 锚点冲突时输出警告

# Phase 3-B（计划中）
--split-output DIR          # 同时输出分文件版本
...
| **Phase 3-A** | T3.A1-A4 锚点冲突修复 | P0 | 📋 计划中 |
| **Phase 3-B** | T3.B1-B4 双版本输出 | P1 | 📋 计划中 |
...
4. **Phase 3-A**：合并版目录跳转正确，无锚点冲突
5. **Phase 3-B**：合并版与分文件版同时产出
...
方案可行，且在保持工具轻量化的前提下，能显著改善 docs/wiki 的可读性问题。**Phase 1/2 已完成**，验证效果良好（OpenClaw 文档从 3.9MB 降至 1.7MB）。下一步重点是 **Phase 3-A（锚点冲突修复）**，确保合并文档的目录跳转正确性。
